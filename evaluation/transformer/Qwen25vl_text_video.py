...
def build_pos_content(video_path, background9, background19, pos, gt_frame_number, bk_frame_number, random_nums, video_dir):
    if pos[:2] == "10":
        pos_num = int(pos[-2:])
        if pos_num == int(pos[:2]):
            ...
            video_list = list(background19.keys())
            video_lists = [video_dir + "/" + s for s in video_list]
            pos_num = int(pos[-2:])
            ...
            for video_path_ in background_list:
                if video_path_ == video_path:
                    ...
                    random_num = random_nums[text_num]
                    if random_num < 0.5:
                        content.append(
                            {
                                "type": "text",
...
def pos_inference(
    ...
    content1 = build_pos_content(video_path, background9, background19, pos, frame_number, bk_frame_number, random_num, video_dir)
...
def run_inference(args):
    ...
    for index, sample in enumerate(tqdm(gt_qa_pairs)):
        ...
        background19 = sample["9_background"]
        random_num = generate_floats(index)
